...
articleView.populateFilters = function () {
  $('article').each(function () {
    ...
    if (!$(this).hasClass('template')) {
      ...
      authorName = $(this).attr('data-attr');
...
articleView.handleAuthorFilter = function () {
  $('#author-filter').on('change', function () {
    ...
    if ($(this).val()) {
      ...
      $('article').hide();
      $('article').filter(`[data-attr="${selectedAuthor}"]`).fadeIn();

    } else {
      $('article').show().filter('.template').hide();
...
articleView.handleCategoryFilter = function () {
  ...
  $('#category-filter').on('change', function () {
    if ($(this).val()) {
      ...
      $('article').hide();
      $('article').filter(`[data-category="${selectedCategory}"]`).show();
    } else {
      $('article').show().filter('.template').hide();
    }
    $('#author-filter').val('');
...
articleView.setTeasers = function() {
  ...
  $('article .read-on').on('click' , function(e) {
    e.preventDefault();
